...
from app.articles.domain import Article, ArticleSummary, SectionItem
...
SELECT * FROM (
    SELECT part_id, part_abbr, question_n, question_title,
           ...
           sed_contra AS text, 'sed_contra' AS section,
           'On the contrary' AS section_label, 'sed-contra' AS url_fragment,
           ts_rank(to_tsvector('english', sed_contra), q.tsq) AS fts_rank
    FROM summa_articles, q
    WHERE sed_contra IS NOT NULL AND to_tsvector('english', sed_contra) @@ q.tsq
    UNION ALL
    SELECT part_id, part_abbr, question_n, question_title,
           article_n, article_title, source_url,
           respondeo AS text, 'respondeo' AS section,
           'I answer that' AS section_label, 'respondeo' AS url_fragment,
           ts_rank(to_tsvector('english', respondeo), q.tsq) AS fts_rank
    FROM summa_articles, q
    WHERE respondeo IS NOT NULL AND to_tsvector('english', respondeo) @@ q.tsq
    UNION ALL
    ...
    WHERE obj->>'text' IS NOT NULL AND to_tsvector('english', obj->>'text') @@ q.tsq
    UNION ALL
    SELECT a.part_id, a.part_abbr, a.question_n, a.question_title,
           a.article_n, a.article_title, a.source_url,
           rep->>'text' AS text,
    ...
    WHERE rep->>'text' IS NOT NULL AND to_tsvector('english', rep->>'text') @@ q.tsq
) sub
...
def _parse_sections(raw) -> list[SectionItem]:
...
def _row_to_article(row: asyncpg.Record) -> Article:
    d = dict(row)
    for key in ("objections", "replies", "objections_la", "replies_la"):
        d[key] = _parse_sections(d.get(key) or [])
    return Article(**d)
...
class ArticleRepository:
    ...
    async def ensure_schema(self) -> None:
        async with self._pool.acquire() as conn:
            ...
            try:
                ...
                logger.warning("Could not create pg_trgm indexes (%s) — search will be slower.", exc)
            try:
                for sql in _FTS_INDEX_SQLS:
                    await conn.execute(sql)
            except Exception as exc:
                logger.warning("Could not create FTS indexes (%s) — full-text search will be slower.", exc)

    async def get_article(self, part_id: str, question_n: int, article_n: int) -> Article | None:
        async with self._pool.acquire() as conn:
    ...
    async def get_articles_for_question(self, part_id: str, question_n: int) -> list[ArticleSummary]:
        async with self._pool.acquire() as conn:
    ...
    async def fts_search(self, query: str, limit: int = 8) -> list[PassageResult]:
        """Full-text search across all article sections. Returns PassageResult
        because FTS is a passage-retrieval operation that feeds into the search pipeline."""
        async with self._pool.acquire() as conn:
