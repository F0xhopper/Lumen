...
CREATE TABLE IF NOT EXISTS user_preferences (
    ...
    text_language   TEXT        NOT NULL DEFAULT 'both',
    updated_at      TIMESTAMPTZ DEFAULT NOW()
);
"""

_ADD_TEXT_LANGUAGE_SQL = """
ALTER TABLE user_preferences
    ADD COLUMN IF NOT EXISTS text_language TEXT NOT NULL DEFAULT 'both';
"""
...
_VALID_TEXT_LANGUAGES = {"both", "en", "la"}
...
def _to_domain(row: asyncpg.Record) -> FontPrefs:
    ...
    return FontPrefs(
        ...
        text_language=row["text_language"] if row.get("text_language") in _VALID_TEXT_LANGUAGES else d.text_language,
    )
...
class PreferencesRepository:
    ...
    async def ensure_schema(self) -> None:
        async with self._pool.acquire() as conn:
            ...
            await conn.execute(_ADD_TEXT_LANGUAGE_SQL)
        ...
        async with self._pool.acquire() as conn:
            row = await conn.fetchrow(
                ...
                INSERT INTO user_preferences
                    (user_id, font_family, font_size, line_height, letter_spacing, font_weight, text_language, updated_at)
